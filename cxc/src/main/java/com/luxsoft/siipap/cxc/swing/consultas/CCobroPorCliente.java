...
public class CCobroPorCliente extends AbstractView{
	...
	content.add(new HeaderPanel("Cobro por Cliente","Cobro Por Cliente deacuerdo a la fecha programada de cobro y facturas con saldo"),BorderLayout.NORTH);
	...
	private EventList createEventList(){
		...
		final FilterList sucursalList=new FilterList(claveList,sucursalEditor);
	...
	private class TF implements TableFormat<Map<String, Object>>{
		...
		String cols[]={"CLAVE","NOMBRE","ORIGEN","SUCURSAL","TIPO","NUMERO","NUMEROFISCAL","FECHA","VENCIMIENTO"
				,"DIAS","SALDO","COBRADOR","DIAPAGO","DIAREVISION"};
	...
	private List loadData(){
		String sql=SQLUtils.loadSQLQueryFromResource("sql/CCobroPorCliente.sql");
	...
	private void updateTotales(){
		...
		for(Object o:eventList){
			...
			BigDecimal dd=(BigDecimal)row.get("SALDO");
			if(dd!=null)
				importe=importe.add(dd);
			BigDecimal d1=(BigDecimal)row.get("SALDO");
			if(d1!=null)
				prov=prov.add(d1);
		}
	...
	public static void main(String[] args) {
		...
		final CCobroPorCliente e=new CCobroPorCliente();
