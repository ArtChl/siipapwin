...
public class CDescuentosPorSucursal extends AbstractView{
	...
	content.add(new HeaderPanel("Descuentos Por sucursal","Consulta Descuentos Por Sucursal"),BorderLayout.NORTH);
	...
	private JComponent buildFilPanel(){
		...
		builder.add(sucursal,cc.xyw(2, 1, 4));
		builder.add(serie,cc.xyw(8, 1, 4));
		builder.add(tipo,cc.xyw(15, 1, 4));
	...
	private EventList createEventList(){
		...
		final TextComponentMatcherEditor sucEditor=new TextComponentMatcherEditor(sucursal,sucursalFilterator);
		final FilterList sucList=new FilterList(sortedList,sucEditor);
		
		final TextFilterator<Map<String, Object>> serieFilterator=new TextFilterator<Map<String,Object>>(){
			
			public void getFilterStrings(List<String> baseList, Map<String, Object> element) {
				String s=element.get("SERIE").toString();
				baseList.add(s);
			}			
		};@SuppressWarnings({ "unchecked"})
		final TextComponentMatcherEditor serieEditor=new TextComponentMatcherEditor(serie,serieFilterator);
		final FilterList serieList=new FilterList(sucList,serieEditor);
		
		final TextFilterator<Map<String, Object>> tipoFilterator=new TextFilterator<Map<String,Object>>(){
			
			public void getFilterStrings(List<String> baseList, Map<String, Object> element) {
				String s=element.get("TIPO_VENTA").toString();
				baseList.add(s);
			}			
		};
		final TextComponentMatcherEditor tipoEditor=new TextComponentMatcherEditor(tipo,tipoFilterator);
		final FilterList tipoList=new FilterList(serieList,tipoEditor);
		this.eventList=tipoList;
		return tipoList;
	}
	
	private class TF implements TableFormat<Map<String, Object>>{
		
		String cols[]={"SUCURSAL","SERIE","PERIODO","TIPO_VENTA","IMP_P_LIST","TIPO_VENTA","IMP_P_LIST"
	...
	private List loadData(){
		String sql=SQLUtils.loadSQLQueryFromResource("sql/CDescuentosPorSucursal.sql");
	...
	public static void main(String[] args) {
		...
		final CDescuentosPorSucursal e=new CDescuentosPorSucursal();
		SXAbstractDialog dialog=new SXAbstractDialog("DESCUENTO POR SUCURSAL"){
