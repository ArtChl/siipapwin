...
public class CDescuentosPorCliente extends AbstractView{
	...
	content.add(new HeaderPanel("Descuentos Por Cliente","Consulta Descuentos Por Cliente"),BorderLayout.NORTH);
	...
	private JComponent buildFilros(){
	 ...
	 private JComponent buildFilPanel(){
		...
		builder.add(new JLabel("Cliente"),cc.xy(1, 1));
		builder.add(new JLabel("Nombre"),cc.xy(7, 1));
		builder.add(new JLabel("Sucursal"),cc.xyw(13, 1, 2));
		builder.add(cliente,cc.xyw(2, 1, 4));
		builder.add(nombre,cc.xyw(8, 1, 4));
		builder.add(sucursal,cc.xyw(15, 1, 4));
	...
	private EventList createEventList(){
		...
		final TextFilterator<Map<String, Object>> clienteFilterator=new TextFilterator<Map<String,Object>>(){
			
			public void getFilterStrings(List<String> baseList, Map<String, Object> element) {
				String s=element.get("CLIENTE").toString();
				baseList.add(s);
			}			
		};
		final TextComponentMatcherEditor clienteEditor=new TextComponentMatcherEditor(cliente,clienteFilterator);
		final FilterList claveList=new FilterList(sortedList,clienteEditor);
	...
	private class TF implements TableFormat<Map<String, Object>>{
		...
		String cols[]={"CLIENTE","NOMBRE","SERIE","SUCURSAL","PERIODO","TIPO_VENTA","IMP_P_LIST"
	...
	private List loadData(){
		String sql=SQLUtils.loadSQLQueryFromResource("sql/CDescuentosPorCliente.sql");
	...
	public static void main(String[] args) {
		...
		final CDescuentosPorCliente e=new CDescuentosPorCliente();
		SXAbstractDialog dialog=new SXAbstractDialog("DESCUENTO POR CLIENTE"){
