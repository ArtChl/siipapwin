...
,ROUND(CASE WHEN A.ORIGEN='CRE' THEN (select x.descuento     from SW_VENTASCREDITO x where a.venta_id=x.venta_id) else 0.45  END ,2) as DESCUENTO
...
,CASE WHEN A.CLAVE='U050008' THEN 0.27 ELSE case when (select x.descuento     from SW_VENTASCREDITO x where a.venta_id=x.venta_id)>=50 then 0.27 else .45  end END as COMISION
