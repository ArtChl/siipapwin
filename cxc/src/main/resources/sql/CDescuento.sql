SELECT A.LINEA,A.CLAVE,C.NOMBRE,A.SERIE,A.SUCURSAL,TO_CHAR(A.FECHA,'MM/YYYY') AS PERIODO
,CASE WHEN A.SERIE='E' THEN 'CREDITO' ELSE 'CONTADO' END AS TIPO_VENTA
...
SUM((-A.CANTIDAD-A.DEVOLUCIONES)*A.PRECIOFACTURADO),0)
END AS DESCTO
,CASE WHEN A.TIPOFACTURA='N' THEN 
ROUND(100-(SUM((((-A.CANTIDAD-A.DEVOLUCIONES)*A.PRECIOFACTURADO)*(100-A.DESCUENTO))/100)*100)/
SUM((-A.CANTIDAD-A.DEVOLUCIONES)*A.PRECIOLISTA),2)
ELSE 0
END AS DESCTO_P_NETO
FROM V_VENTASDET A
JOIN SW_CLIENTES C ON(A.CLIENTE=C.CLAVE)
WHERE FECHA BETWEEN '01/01/2007' AND '31/01/2007'
GROUP BY A.LINEA,A.CLAVE,C.NOMBRE,A.SERIE,A.TIPOFACTURA,A.SUCURSAL,TO_CHAR(A.FECHA,'MM/YYYY')
