...
/**
 *  Vista interna para los beans {@link SalidaACorte} en proceso
 *  
 * @author Ruben Cancino
 *
 */
@SuppressWarnings("unchecked")
public class SalidasACorteEnProcesoView extends AbstractInternalTaskView implements Totalizable{
	...
	private FilterList<SalidaACorte> salidasFiltradas;
	
	public SalidasACorteEnProcesoView(final EventList<EntradaDeMaterial> entradas,final EventList<SalidaACorte> salidas){		
		setTitle("En Proceso");
	...
	public void totalizar(TotalizadorDeMaquila totalizador) {
		...
		for(SalidaACorte s:salidasFiltradas){
			kilos=kilos.add(s.getKilos());
			pesos=pesos.add(s.getCostoAsMoneda());	
		}
		totalizador.setProcesoEnKilos(kilos);
		totalizador.setImporteProceso(pesos);
