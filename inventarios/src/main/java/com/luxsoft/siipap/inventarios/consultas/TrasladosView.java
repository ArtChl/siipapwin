...
/**
 *  Vista interna de las salidas de material
 *  
 * @author Ruben Cancino
 *
 */
public class TrasladosView extends AbstractInternalTaskView implements Totalizable{
	
	private JXTable grid;
	private final EventList<EntradaDeMaterial> entradas;
	private final EventList<SalidaDeMaterial> salidas;
	private FilterList<SalidaDeMaterial> salidasFiltradas;
	
	public TrasladosView(final EventList<EntradaDeMaterial> entradas,final EventList<SalidaDeMaterial> salidas){		
		setTitle("Traslados");
	...
	private JComponent createGridPanel(){
		...
		salidasFiltradas=new FilterList<SalidaDeMaterial>(salidas,new ThreadedMatcherEditor<SalidaDeMaterial>(editor));
		
		final SortedList<SalidaDeMaterial> sortedList=new SortedList<SalidaDeMaterial>(salidasFiltradas,null);
		
		final EventTableModel<SalidaDeMaterial> tm=new EventTableModel<SalidaDeMaterial>(
				sortedList,InventarioUtils.getTrasladosTF());
		final EventSelectionModel<SalidaDeMaterial> sm=new EventSelectionModel<SalidaDeMaterial>(sortedList);
		
		grid=ComponentUtils.getStandardTable();
		grid.setModel(tm);
		grid.setSelectionModel(sm);
		new TableComparatorChooser<SalidaDeMaterial>(grid,sortedList,true);
	...
	public void totalizar(TotalizadorDeMaquila totalizador) {
		...
		totalizador.setTrasladosEnKilos(kilos);
		totalizador.setImporteTraslados(pesos);
	}
	
	/**
	 * Matcher para filtrar beans {@link SalidaDeMaterial} en funcion de una lista filtrada de {@link EntradaDeMaterial}
	 * 
	 * @author Ruben Cancino
	 *
	 */
	private class SalidaDeBobinaMatcher extends AbstractMatcherEditor<SalidaDeMaterial> implements ListEventListener<EntradaDeMaterial>{
		...
		private class BobinasMatcher implements Matcher<SalidaDeMaterial>{
